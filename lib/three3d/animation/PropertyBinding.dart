...
class PropertyBinding {
  ...
  static create( root, path, parsedPath ) {
		...
		if ( ! ( root != null && root.runtimeType.toString() == "AnimationObjectGroup" ) ) {
  ...
  setValue_fromArray_setMatrixWorldNeedsUpdate( buffer, offset ) {
	...
	bind() {
		...
		if ( targetObject == null ) {
			...
			print( 'THREE.PropertyBinding: Trying to update node for track: ' + this.path + ' but it wasn\'t found.' );
			return;
    ...
    } else if ( ["Color", "Vector3", "Quaternion"].indexOf( nodeProperty.runtimeType.toString() ) >= 0 ) {
