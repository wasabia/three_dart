  ...
  static createShapes( shapePath ) {
    ...
    classifyPoint( p, edgeStart, edgeEnd ) {
		...
		getIntersections( path1, path2 ) {
			...
			for ( var index = 1; index < path1.length; index ++ ) {
				...
				for ( var index2 = 1; index2 < path2.length; index2 ++ ) {
					...
					if ( intersection != null && intersectionsRaw.indexWhere( (i) => i["t"] <= intersection["t"] + Math.EPSILON && i["t"] >= intersection["t"] - Math.EPSILON ) < 0 ) {

						intersectionsRaw.add( intersection );
						intersections.add( new Vector2( intersection["x"], intersection["y"] ) );
      ...
      return sp["points"].length > 1;
		...
		var isAHole = simplePaths.map( (p) => isHoleTo( p, simplePaths, scanlineMinX, scanlineMaxX, shapePath.userData["style"]["fillRule"] ) ).toList();
